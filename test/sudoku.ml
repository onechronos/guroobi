    ...
    for i = 0 to dim - 1 do
      try
        ...
        for j = 0 to dim - 1 do
          ...
          if char_value < 0 || char_value >= dim then
            board.(i).(j) <- -1
          else
            board.(i).(j) <- char_value
        done
      with End_of_file ->
    ...
    for i = 0 to dim - 1 do
      for j = 0 to dim - 1 do
        for v = 0 to dim - 1 do
          ...
          names.(i * dim * dim + j * dim + v) <- sp "x[%d,%d,%d]" i j (v + 1)
        done
      done
    done;
      ...
      let model =
        eer "new_model"
          (new_model ~env ~name:(Some "sudoku") ~num_vars:(dim * dim * dim) ~objective:None
             ~lower_bound:(Some lb) ~upper_bound:None ~var_type:(Some vtype) ~var_name:(Some names))
      in
      ...
      for i = 0 to dim - 1 do
        for j = 0 to dim - 1 do
          ...
          az (add_constr ~model ~num_nz:dim ~var_index:ind ~nz:value 
