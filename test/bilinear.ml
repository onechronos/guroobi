...
(* This example formulates and solves the following simple bilinear model:
   ...
   maximize x subject to x + y + z <= 10 x * y <= 2 (bilinear inequality) x * z
   + y * z == 1 (bilinear equality) x, y, z non-negative (x integral in second
   version) *)
...
let main () =
  ...
  | Ok () ->
      ...
      az
        (set_char_attr_element ~model ~name:GRB.char_attr_vtype ~index:0
           ~value:GRB.integer);
