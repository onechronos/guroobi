...
let main () =
  ...
  | Ok () ->
      ...
      let model =
        eer "new_mode"
          (new_model ~env ~name:(Some "genconstr") ~num_vars:n_vars
      ...
      for i = 0 to n_literals - 1 do
        ...
        az
          (set_char_attr_element ~model ~name:"VType" ~index:col
             ~value:GRB.binary);
        az
          (set_str_attr_element ~model ~name:"VarName" ~index:col ~value:buffer);
      ...
      for i = 0 to n_literals - 1 do
        ...
        cval.{1} <- 1.;
        az
          (add_constr ~model ~num_nz:2 ~var_index:cind ~nz:cval ~sense:GRB.equal
             ~rhs:1.0 ~name:(Some buffer))
      done;

      for i = 0 to n_clauses - 1 do
        let buffer = sp "CNSTR_Clause%d" i in
        az
          (add_gen_constr_or ~model ~name:(Some buffer) ~res_var:(cla i)
             ~n_vars:3
             ~vars:(to_i32a clauses.(i)))
      done;
      ...
      az
        (add_gen_constr_min ~model ~name:(Some "CNSTR_Obj0") ~res_var:(obj 0)
           ~n_vars:n_clauses ~vars:cind ~constant:GRB.infinity);
      az
        (add_gen_constr_indicator ~model ~name:(Some "CNSTR_Obj1")
           ~bin_var:(obj 1) ~bin_val:1 ~n_vars:n_clauses ~ind:(Some cind)
           ~value:(Some cval) ~sense:GRB.greater_equal ~rhs:4.0);
      ...
      then
        ...
        in
        if obj_val > 1.9 then pr "Logical expression is satisfiable\n"
        else if obj_val > 0.9 then pr "At least four clauses can be satisfied\n"
        else pr "At most three clauses may be satisfied\n"
