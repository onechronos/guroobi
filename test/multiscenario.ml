...
(* Facility location: a company currently ships its product from 5 plants
   ...
   Note that this example is similar to the facility.ml example. Here we
...
let main () =
  ...
  | Ok () ->
      ...
      let model =
        eer "new_mode"
          (new_model ~env ~name:(Some "multiscenario") ~num_vars:(n_plants * (n_warehouses + 1)) 
      ...
      for p = 0 to n_plants - 1 do
        if fixed_costs.(p) > fixed_costs.(!max_index) then
          max_index := p
        else if fixed_costs.(p) < fixed_costs.(!min_index) then
          min_index := p
      done;
      ...
      az (set_str_attr ~model ~name:GRB.str_attr_scennname ~value:"Increased warehouse demands");

      for w = 0 to n_warehouses - 1 do
        az (set_float_attr_element ~model ~name:GRB.dbl_attr_scennrhs ~index:(demandconstr w) ~value:(demand.(w) *. 1.1));
      done;

      (* Scenario 2: Double the warehouse demands *)
      az (set_int_model_param ~model ~name:GRB.int_par_scenarionumber ~value:2);
      az (set_str_attr ~model ~name:GRB.str_attr_scennname ~value:"Double the warehouse demands");

      for w = 0 to n_warehouses - 1 do
        az (set_float_attr_element ~model ~name:GRB.dbl_attr_scennrhs ~index:(demandconstr w) ~value:(demand.(w) *. 2.0));
      done;

      (* Scenario 3: Decrease the plant fixed costs by 5% *)
      az (set_int_model_param ~model ~name:GRB.int_par_scenarionumber ~value:3);
      az (set_str_attr ~model ~name:GRB.str_attr_scennname ~value:"Decreased plant fixed costs");

      for p = 0 to n_plants - 1 do
        az (set_float_attr_element ~model ~name:GRB.dbl_attr_scennobj ~index:(opencol p) ~value:(fixed_costs.(p) *. 0.95));
      done;

      (* Scenario 4: Combine scenario 1 and scenario 3 *)
      az (set_int_model_param ~model ~name:GRB.int_par_scenarionumber ~value:4);
      az (set_str_attr ~model ~name:GRB.str_attr_scennname ~value:"Increased warehouse demands and decreased plant fixed costs");
      ...
      az (set_str_attr ~model ~name:GRB.str_attr_scennname ~value:"Force plant with largest fixed cost to stay open");

      az (set_float_attr_element ~model ~name:GRB.dbl_attr_scennlb ~index:(opencol !max_index) ~value:1.0);

      (* Scenario 6: Force the plant with the smallest fixed cost to be closed *)

      az (set_int_model_param ~model ~name:GRB.int_par_scenarionumber ~value:6);
      az (set_str_attr ~model ~name:GRB.str_attr_scennname ~value:"Force plant with smallest fixed cost to be closed");

      az (set_float_attr_element ~model ~name:GRB.dbl_attr_scennub ~index:(opencol !min_index) ~value:0.0);
      ...
      for p = 0 to n_plants - 1 do
        az (set_float_attr_element ~model ~name:GRB.dbl_attr_start ~index:(opencol p) ~value:1.0);
      done;
      ...
      for s = 0 to n_scenarios - 1 do
        ...
        az (set_int_model_param ~model ~name:GRB.int_par_scenarionumber ~value:s);
        ...
        let scen_n_obj_bound =
          eer "get_float_attr" (get_float_attr ~model ~name:GRB.dbl_attr_scennobjbound)
        in
        
        let scen_n_obj_val =
          eer "get_float_attr" (get_float_attr ~model ~name:GRB.dbl_attr_scennobjval)
        in
        pr "\n\n------ Scenario %d (%s)\n" s scenario_name;
        ...
        if ((float_of_int GRB.minimize) *. scen_n_obj_val) >= GRB.infinity then (
          ...
          for p = 0 to n_plants - 1 do
            let scen_n_x =
              eer "get_float_attr_element" (get_float_attr_element ~model ~name:GRB.dbl_attr_scennx ~index:(opencol p))
            in
            if scen_n_x > 0.5 then (
              pr "Plant %d open\n" p;
              for w = 0 to n_warehouses - 1 do
                let scen_n_x =
                  eer "get_float_attr_element" (get_float_attr_element ~model ~name:GRB.dbl_attr_scennx ~index:(transportcol w p))
                in
                if scen_n_x > 0.0001 then
                  ...
                  if floor rounded = rounded then
                    ...
                    pr "  Transport %.1f units to warehouse %d\n" scen_n_x w
