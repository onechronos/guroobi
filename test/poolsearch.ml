...
let main () =
  ...
  | Ok () ->
      ...
      let model =
        eer "new_model"
          (new_model ~env ~name:(Some "poolsearch") ~num_vars:ground_set_size
      ...
      az
        (set_float_attr_array ~model ~name:"Obj" ~start:0 ~len:ground_set_size
           ~values:(to_fa obj_coef));

      (* set variable types and names *)
      for e = 0 to ground_set_size - 1 do
        let buffer = sp "El%d" e in
        az
          (set_char_attr_element ~model ~name:"VType" ~index:e ~value:GRB.binary);
      ...
      az
        (add_constr ~model ~num_nz:ground_set_size ~var_index:c_ind
           ~nz:(to_fa knapsack_coef) ~sense:GRB.less_equal ~rhs:budget
           ~name:(Some buffer));
      ...
      az
        (set_int_model_param ~model ~name:GRB.int_par_poolsolutions ~value:1024);
      ...
      then
        ...
        for e = 0 to n_solutions - 1 do
          ...
          let obj_val =
            eer "get_float_attr"
              (get_float_attr ~model ~name:GRB.dbl_attr_poolobjval)
          in
        ...
        if n_solutions >= 4 then (
          ...
          let c_val =
            eer "get_float_attr_array"
              (get_float_attr_array ~model ~name:GRB.dbl_attr_xn ~start:0
                 ~len:ground_set_size)
          in
