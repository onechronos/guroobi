  ...
  | Ok () ->
      ...
      let model =
        eer "read_model"
          ...
          | FileNotFound -> pr "Error: unable to open input file\n";
            exit 1
          | Ok m -> Ok m
      ...
      let num_vars =
        eer "get_int_attr"
          (get_int_attr ~model ~name:"NumVars")
      in
      let num_int_vars =
        eer "get_int_attr"
          (get_int_attr ~model ~name:"NumIntVars")
      in
      ...
      for j = 0 to num_vars - 1 do
        ...
        if v_type <> GRB.continuous then (
          ...
          incr num_fractional;
          az (set_char_attr_element ~model ~name:"VType" ~index:j ~value:GRB.continuous)
        )
      done;
      ...
      let rec for_loop iter =
        ...
        let obj =
          eer "get_float_attr"
            (get_float_attr ~model ~name:"ObjVal")
        in
        pr "Iteration %i, obj %f, fractional %i\n" iter obj !num_fractional;

        if !num_fractional = 0 then pr "Found feasible solution - objective %f\n" obj
        else (
          ...
          for j = 0 to n_fix - 1 do
            ...
            az (set_float_attr_element ~model ~name:"LB" ~index:fractional_filled.(j).index ~value:fix_val);
            az (set_float_attr_element ~model ~name:"UB" ~index:fractional_filled.(j).index ~value:fix_val);
            let vname = 
              eer "get_str_attr_element"
                (get_str_attr_element ~model ~name:"VarName" ~index:fractional_filled.(j).index)
            in
            pr "  Fix %s to %f ( rel %f )\n" vname fix_val fractional_filled.(j).x
          done;
          az (optimize model);
